<template>
    <div id="selfMes">
        ...
        <div class="userMesLine" flex="dir:left main:justify">
            ...
            <ul>
                <li flex="dir:left">昵称:<p style="width: 300px" class="oneLine">{{userInfo.nick_name}}</p></li>
        ...
        <div class="addressList" flex="dir:left">
            <div v-for="(address,index) in addressList">
                <div class="address">
                    ...
                    <div>
                        <span class="pink hand" @click="saveAddressId(address)">修改</span>
                        <span class="hand" @click="delAddress(index)">删除</span>
                    </div>
                </div>
            </div>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods: {
            ...
            changeAddressList(){
                setTimeout(()=>{
                    let addressList = util.getStorage('addressList',localStorage)
            ...
            delAddress(index){
                util.ajaxPost(this, '1.0', 'user/del_address', {'auth_token': AUTH_TOKEN,"id": this.addressList[index].id}, (res) => {
                    if (res.body.code == 0) {
                        this.$store.commit("ALERT_CHANGE", {
                            status: true,
                            errText: "删除成功",
