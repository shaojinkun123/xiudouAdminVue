let util = {
    ...
    ajaxGet(that, version, url, params, successFun)
    {
        ...
        that.$http.get('api/' + this.URL_INTERFACE(version, url), {
                params: params
            })
            .then(
                successFun,
    ...
    {
        return '?version=' + version + '&request_url=' + interfaceName + '&source=pc&request_time=' + new Date().getTime();
    }
    ,
    ajaxPost(that, version, url, params, successFun)
    {
       ...
       // const  API_PROXY = 'https://m.xiudou.net/api/index.php'             //线上地址
        that.$http.post('api/' + this.URL_INTERFACE(version, url), params,
            {emulateJSON: true})
            .then(
                successFun,
                (res) => {
